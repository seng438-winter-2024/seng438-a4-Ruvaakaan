...
|      isNaNRange()          |    return Double.isNaN(this.lower) && Double.isNaN(this.upper)  |  deincremented (--a) double field upper |  survived   |   Our test case with Range(Double.NaN, Double.NaN) fails to kill this mutation. The expected return is true as the range is between NaNs. The upper bounds is deincremented. Although this does not change the test result since you cannot deincrement NaN values. Thus, the range stays as a NaN range.   | 
|      isNaNRange()          |    if (!contains(value))  |  removed conditional replaced equality check with false |  survived   |   Our test case with Range(0.0, 10.0) and value = 15.0 kills this mutation. The expected return is 10 since the value falls outside the upper range. The removed conditional mutation always returns the value (15 in this case). Thus, the test catches and kills the mutation.   | 
