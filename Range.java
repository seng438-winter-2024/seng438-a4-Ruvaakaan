...
//    CHANGED
    public boolean intersects(double b0, double b1) {
        if (b0 <= this.upper && b1 >= this.lower) {
            return true; // Ranges intersect
        } else {
            return false; // Ranges don't intersect
        }
    }
...
//    CHANGED
    public static Range combine(Range range1, Range range2) {
        if (range1 == null && range2 == null) {
            return null; // If both ranges are null, return null
        } else if (range1 == null) {
            return range2; // If range1 is null, return range2
        } else if (range2 == null) {
            return range1; // If range2 is null, return range1
        } else {
